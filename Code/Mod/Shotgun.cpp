...
bool CShotgun::Shoot(bool resetAnimation, bool autoreload/* =true */, bool noSound /* =false */)
{
	...
	if (m_pWeapon->IsServer())
		g_pGame->GetIGameFramework()->GetIGameplayRecorder()->Event(m_pWeapon->GetOwner(),
			GameplayEvent(eGE_WeaponShot, ammo->GetName(), m_shotgunparams.pellets,
				reinterpret_cast<void*>(static_cast<uintptr_t>(m_pWeapon->GetEntityId()))));
